    ...
    <div className="min-h-screen pb-20 font-sans transition-colors duration-300">
      ...
      <main className="w-full max-w-[1800px] mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
        ...
        {activeTab === 'dashboard' && (() => {
          ...
          // Agrupar prestadores por NIT para el panel izquierdo
                ...
                {dashGrupos.map(([nit, grupo]) => {
                  ...
                  return (
                    <div key={nit} className="rounded-xl border border-slate-200 dark:border-slate-700/60 overflow-hidden bg-slate-50/50 dark:bg-slate-800/30">
                      {/* Header del grupo — clic para abrir/cerrar */}
                      <button
                        onClick={() => setDashExpandedNits(prev => {
                      ...
                      {isOpen && (
                        <div className="border-t border-slate-200 dark:border-slate-700 divide-y divide-slate-100 dark:divide-slate-800">
                          {grupo.map(p => {
                            ...
                            const isContrib = p.regimen === 'CONTRIBUTIVO';
                            const regimenColor = isContrib ? 'text-orange-600 dark:text-orange-400' : 'text-emerald-600 dark:text-emerald-400';
                            const regimenBg = isContrib ? 'bg-orange-50 dark:bg-orange-500/10 border-orange-100 dark:border-orange-500/20' : 'bg-emerald-50 dark:bg-emerald-500/10 border-emerald-100 dark:border-emerald-500/20';
                            return (
                              <div key={p.id} className="px-3 py-2.5 space-y-1.5">
                                <div className="flex items-center justify-between">
                                  <div className={`flex items-center gap-2 px-2 py-1 rounded-lg border text-[10px] font-medium ${regimenBg}`}>
