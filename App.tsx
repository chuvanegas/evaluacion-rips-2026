        ...
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
          ...
          <section className="glass-panel rounded-2xl p-5 shadow-xl relative overflow-hidden">
            ...
            {prestadoresAsistencial.length === 0 ? (
              ...
              // Filtrar y agrupar por NIT
              const q = searchPrestador.toLowerCase();
              const filtrados = prestadoresAsistencial.filter(p =>
                !q || p.nombre.toLowerCase().includes(q) || p.contrato.toLowerCase().includes(q) || (p.nit || '').includes(q)
              );
              const grupos = new Map<string, Prestador[]>();
              filtrados.forEach(p => {
                  ...
                  {Array.from(grupos.entries()).map(([nit, grupo]) => {
                    ...
                    return (
                      <div key={nit} className="rounded-xl border border-slate-200 dark:border-slate-700/60 overflow-hidden bg-slate-50/50 dark:bg-slate-800/30">
                        ...
                        {/* Cajón: contratos + actas */}
                        {isOpen && (
                          <div className="border-t border-slate-200 dark:border-slate-700 divide-y divide-slate-100 dark:divide-slate-800">
                            {grupo.map(p => {
                              const pActas = [...new Map(
                                actas.filter(a => a.prestadorId === p.id || (a.nit === p.nit && a.contrato === p.contrato))
                                .map(a => [a.id, a])
                              ).values()];
                              const regimenColor = p.regimen === 'CONTRIBUTIVO' ? 'text-orange-500' : 'text-emerald-600';
                              const regimenBg = p.regimen === 'CONTRIBUTIVO' ? 'bg-orange-50 dark:bg-orange-500/10 border-orange-100 dark:border-orange-500/20' : 'bg-emerald-50 dark:bg-emerald-500/10 border-emerald-100 dark:border-emerald-500/20';
                              return (
                                <div key={p.id} className="px-3 py-2 space-y-1.5">
                                  {/* Contrato header */}
                                  <div className="flex items-center justify-between">
                                    <div className={`flex items-center gap-2 px-2 py-1 rounded-lg border text-[10px] ${regimenBg}`}>
                                      <span className="font-mono text-slate-600 dark:text-slate-300">📋 {p.contrato}</span>
                                      <span className={`font-bold ${regimenColor}`}>{p.regimen || 'SUBSIDIADO'}</span>
                                    </div>
                                    <button
                                      onClick={e => { e.stopPropagation(); handleGenerarActa(p); }}
                                      className="text-[10px] px-2 py-1 bg-indigo-600 hover:bg-indigo-700 text-white rounded-lg transition-colors font-medium"
                                    >
                                      + Acta
                                    </button>
                                  </div>
                                          ...
                                          <div key={a.id} className="flex items-center justify-between bg-white dark:bg-slate-900/60 rounded-lg px-2 py-1 border border-slate-100 dark:border-slate-800">
                                            ...
                                            <div className="flex items-center gap-1.5 shrink-0">
                                              <div className="w-10 h-1 bg-slate-200 dark:bg-slate-700 rounded-full">
                                                <div className="h-1 rounded-full" style={{ width: `${cumpl}%`, backgroundColor: color }} />
                                              </div>
                                              <span className="text-[10px] font-bold w-8 text-right" style={{ color }}>{cumpl}%</span>
                                              <button
                                                onClick={e => { e.stopPropagation(); setInlineActa(sanitizeActaServicios({ ...a })); setActiveTab('actas'); }}
                                                className="text-[9px] px-1.5 py-0.5 bg-slate-100 dark:bg-slate-800 hover:bg-indigo-100 dark:hover:bg-indigo-500/20 text-slate-500 hover:text-indigo-600 dark:hover:text-indigo-300 rounded transition-colors"
                                              >Ver</button>
                                            </div>
                                          </div>
                                        );
        ...
        {isAdmin && metas.some(m => m.active) && (
          <section className="glass-panel rounded-2xl p-5 shadow-xl animate-in fade-in duration-300">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6 gap-3">
              {(() => {
                // Usar chartData como fuente de verdad: los mismos servicios que aparecen
                // en la gráfica aparecen aquí — sin importar el tipo de contrato detectado.
                const chartTypes = new Set(chartData.map(d => d.name));
                return metas.filter(m => m.active && (chartTypes.has(m.type) || (typeCount[m.type] || 0) > 0)).map((m, idx) => {
                  const ripsCount = typeCount[m.type] || 0;
                  const ren = m.renuencias || 0;
                  const total = ripsCount + ren;
                  return (
                    <div key={idx} className="flex flex-col gap-1.5 bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 rounded-xl p-3">
                      ...
                      <div className="flex items-center gap-1 text-xs text-slate-500 dark:text-slate-400">
                        <span className="font-mono text-slate-700 dark:text-slate-300">{ripsCount.toLocaleString()}</span>
                        <span>RIPS</span>
                        {ren > 0 && <span className="text-amber-500 font-bold">+{ren.toLocaleString()} ren.</span>}
                      </div>
                      <input
                        type="number" min="0"
                        value={ren}
